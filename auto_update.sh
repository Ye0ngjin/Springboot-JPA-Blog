...
PID=$(ps aux | grep "blog-0.0.1-SNAPSHOT" | grep -v "grep" | awk '{print $2}')
if [ -n "$PID" ]; then
...
chmod 775 target/blog-0.0.1-SNAPSHOT.jar | tee -a "target/$LOGFILE"
